...
6. **Создать бакеты (папки для файлов) через MinIO Console:**

Открыть `http://111.88.113.107:9001`, залогиниться и создать бакеты:
...
7. **Настроить CORS для загрузки с фронтенда:**
...
```bash
# На VPS #1 (levelup-app-01):
mkdir -p /home/deploy/levelup-platform/traefik
...
LIVEKIT_API_SECRET=сюда_вставьте_секрет_из_шага_1_выше
...
1. **Зарегистрировать домены (.ru):**
   - `levelup-platform.ru` — Платформа
   - `levelup-academy.ru` — Академия
   - `levelup-association.ru` — Ассоциация (на будущее)
...
levelup-platform.ru          A    111.88.113.107
www.levelup-platform.ru      A    111.88.113.107
api.levelup-platform.ru      A    111.88.113.107
levelup-academy.ru        A    111.88.113.107
www.levelup-academy.ru    A    111.88.113.107
*.levelup-academy.ru      A    111.88.113.107    # Wildcard для школ!
levelup-association.ru    A    111.88.113.107
# LiveKit WebSocket — на VPS #2 (медиасервер)
livekit.levelup-platform.ru  A    111.88.113.71
turn.levelup-platform.ru     A    111.88.113.71
```

> **Обратите внимание:** все веб-домены указывают на VPS #1 (приложение). Отдельные поддомены `livekit.` и `turn.` указывают на VPS #2 (медиасервер). Фронтенд будет подключаться к видеосерверу через `wss://livekit.levelup-platform.ru`.
...
`*.levelup-academy.ru` означает, что ЛЮБОЙ поддомен (my-school.levelup-academy.ru, best-coach.levelup-academy.ru) будет указывать на ваш VPS. Это ключевой элемент мультитенантности: каждая школа получает свой поддомен автоматически, без ручной настройки DNS.
