...
- **Selectel DNS API** — нужен для автоматического получения wildcard SSL-сертификатов (*.academy-coaching.ru) через Traefik
...
Сеть:             Публичная подсеть (авто) — вы получите внешний IP
                  + Приватная подсеть — создайте «score-internal» (10.0.0.0/24)
...
Сеть:             Публичная подсеть (авто)
                  + Приватная подсеть → та же «score-internal» (10.0.0.0/24)
...
# Должно показать что-то вроде: inet 10.0.0.2/24

# На VPS #2:
ip addr show | grep 10.0.0
# Должно показать: inet 10.0.0.3/24

# Проверить связь:
# С VPS #1:
ping 10.0.0.3    # → должен отвечать (VPS #2)
# С VPS #2:
ping 10.0.0.2    # → должен отвечать (VPS #1)
```

> **Важно:** Далее в гайде везде, где написано `<IP-VPS-1>` или `<IP-VPS-2>`, используйте **приватные IP** (10.0.0.x) для связи между серверами, и **публичные IP** для настройки DNS и доступа из интернета. Приватная сеть бесплатна и быстрее.
...
2. Добавьте зоны: `score-coaching.ru`, `academy-coaching.ru`
...
- [ ] Оба VPS созданы и запущены (статус «Активен» в панели)
- [ ] Оба VPS в **одном регионе и одной приватной подсети**
- [ ] Вы записали: публичный IP VPS #1, публичный IP VPS #2, приватный IP VPS #1, приватный IP VPS #2
- [ ] SSH-подключение работает: `ssh root@<публичный-IP-VPS-1>`
- [ ] Приватная сеть работает: ping между серверами проходит
- [ ] DNS-зоны добавлены в Selectel DNS
- [ ] API-ключ Selectel получен и сохранён
- [ ] NS-серверы у регистратора сменены на Selectel
...
**На VPS #1 (приложение, score-app-01):**
```bash
# Обновить систему
...
ssh-copy-id deploy@<публичный-IP-VPS-1>
...
**На VPS #2 (медиасервер, score-media-01):**
```bash
# Базовая настройка — такая же, как VPS #1:
sudo apt update && sudo apt upgrade -y
adduser deploy
usermod -aG sudo deploy
ssh-copy-id deploy@<публичный-IP-VPS-2>
...
Серверы общаются через приватную сеть Selectel (подсеть `score-internal`, 10.0.0.0/24). Трафик по приватной сети бесплатный и быстрый (<1 мс задержки). LiveKit Egress (VPS #2) записывает видео и отправляет на MinIO (VPS #1). API Gateway (VPS #1) генерирует LiveKit-токены, обращаясь к LiveKit API (VPS #2).

```bash
# Проверить связь через ПРИВАТНУЮ сеть:
# С VPS #1 (score-app-01):
ping 10.0.0.3    # Приватный IP VPS #2

# С VPS #2 (score-media-01):
ping 10.0.0.2    # Приватный IP VPS #1
curl http://10.0.0.2:9000  # Проверка доступности MinIO по приватной сети

# Если не работает — проверьте в панели Selectel:
# Облачная платформа → Сети → score-internal
...
1. **Зарегистрировать домены (.ru):**
   ...
   Регистраторы: reg.ru или nic.ru (домены .ru нельзя зарегистрировать в Selectel, но DNS-хостинг — в Selectel).

2. **Настроить DNS-записи в Selectel DNS** (Панель → Сеть и CDN → DNS-хостинг → ваша зона):
...
# Используется Traefik для DNS-01 challenge (wildcard SSL *.academy-coaching.ru)

# ─── Общее ─────────────────────────────────────
NODE_ENV=production
VPS1_EXTERNAL_IP=<публичный IP VPS #1>
VPS1_PRIVATE_IP=<приватный IP VPS #1, напр. 10.0.0.2>
VPS2_PRIVATE_IP=<приватный IP VPS #2, напр. 10.0.0.3>
```
...
VPS2_EXTERNAL_IP=<публичный IP VPS #2>
VPS2_PRIVATE_IP=<приватный IP VPS #2, напр. 10.0.0.3>
...
MINIO_ENDPOINT=http://10.0.0.2:9000  # Приватный IP VPS #1!
...
```
VPS #1 — score-app-01 (Selectel Cloud Server, линейка «Стандарт»):
- 8 vCPU
- 32 GB RAM
- 500 GB NVMe SSD (локальный диск)
- Ubuntu 22.04 LTS, 1 Gbps
- Регион: ru-1 (СПб) или ru-3 (МСК)
- Приватная сеть: score-internal (10.0.0.0/24)
...
VPS #2 — score-media-01 (Selectel Cloud Server, линейка «Стандарт»):
- 8 vCPU
- 16 GB RAM
