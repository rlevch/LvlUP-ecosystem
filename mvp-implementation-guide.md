...
- Сайдбар (260px):
  ...
  - Активный пункт меню: золотая вертикальная полоска слева (3px, ::before), фон rgba(gold, 0.12)
  - Аватар внизу с зелёным пульсирующим индикатором «онлайн» (animation pulse)
- Карточки статистики: цветная полоска сверху (3px), анимация появления снизу (translateY), числа — DM Serif Display крупным, метки — uppercase мелким
- Кнопки: два стиля — primary (золотой фон, белый текст, shadow) и secondary (белый фон, серая рамка)
- Таблицы: подсветка строки при hover (rgba gold 0.07)
- Хлебные крошки (breadcrumbs) под заголовком
- Прогресс-бар дня: gradient от золотого к зелёному
- Карточки «Ближайшие»: при hover — золотая полоска слева (::before)
- Empty state: иконка + заголовок DM Serif Display + описание + CTA кнопка
- Анимации: fade-up при появлении карточек (staggered по 70ms)
- Мобильная версия (≤768px): сайдбар заменяется на sticky top-bar с логотипом и аватаром
