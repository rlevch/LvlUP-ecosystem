...
- Стек: React 19, TypeScript, Tailwind CSS v4, shadcn/ui, React Router v7
...
**2. Генерация экранов для Платформы (levelup-platform.ru):**
...
2. Hero-секция: крупный заголовок "Найдите своего коуча", подзаголовок "Платформа для профессионального коучинга с видеосессиями, играми и инструментами роста", поле поиска коучей, CTA-кнопка "Найти коуча"
3. Блок преимуществ (3 карточки): "Видеосессии с записью", "Игровые инструменты (МАК-карты, кубики)", "Безопасность данных (152-ФЗ)"
4. Каталог коучей: горизонтальный скролл карточек коучей (фото, имя, специализация, рейтинг, цена за сессию, кнопка "Записаться")
5. Как это работает: 4 шага (Регистрация → Выбор коуча → Видеосессия → Результат)
6. Блок отзывов: карусель отзывов клиентов
...
- Кнопка "Войти" (основная, золотой цвет)
- Разделитель "или"
- Кнопка "Войти через VK ID" (синяя, с иконкой VK)
- Ссылка "Забыли пароль?"

Таб "Регистрация":
- Выбор роли: два кликабельных блока "Я коуч" (иконка briefcase) и "Я клиент" (иконка user)
...
- Сайдбар слева (сворачиваемый на мобильных): логотип, пункты меню с иконками:
  ...
  - Библиотека (BookOpen)
  - Настройки (Settings)
- Хедер: поиск, иконка уведомлений (с бейджем количества), аватар с dropdown-меню

Основная область — 4 виджета:
1. "Ближайшие сессии" — список из 3 карточек: имя клиента, дата/время, тип (видео/чат), кнопка "Начать"
2. "Статистика за месяц" — 4 числовых показателя: сессий проведено, новых клиентов, заработано (₽), средний рейтинг
3. "Активные клиенты" — таблица: имя, последняя сессия, следующая сессия, прогресс (progress bar)
4. "Уведомления" — список последних 5 уведомлений (иконка + текст + время)
```
...
Основная область:
1. "Предстоящие сессии" — карточки с фото коуча, датой, кнопкой "Подключиться"
2. "Мои коучи" — горизонтальный скролл карточек (фото, имя, специализация, кнопка "Записаться")
3. "Прогресс" — визуализация (график или progress bars): цели, количество проведённых сессий, достижения
4. "Рекомендации" — карточки рекомендованных коучей, материалов из магазина
```

**Промпт 5 — Профиль коуча (публичный):**
```
Создай публичную страницу профиля коуча для платформы LevelUP на русском языке.

Структура:
1. Шапка профиля: большое фото коуча, имя, специализации (теги), рейтинг (звёзды + число отзывов), город, опыт (лет), статус верификации (зелёная галочка)
2. Блок "О себе": текст-описание коуча, образование, сертификаты (с иконками)
3. Услуги и цены: таблица/карточки — название услуги, длительность, цена (₽), кнопка "Записаться"
4. Расписание: мини-календарь с доступными слотами на текущую/следующую неделю
5. Отзывы клиентов: список отзывов (аватар, имя, дата, рейтинг, текст)
6. Боковая панель (sticky на десктопе): кнопка "Записаться на сессию", цена, ближайшее свободное время
```

**3. Генерация экранов для Академии (levelup-academy.ru):**
...
4. Каталог школ: сетка карточек школ (логотип, название, описание, количество курсов, количество студентов, кнопка "Перейти")
5. Тарифы: 3 колонки (Старт — бесплатно/до 50 студентов, Про — до 500 студентов, Бизнес — безлимит)
6. Футер
...
- Название школы (input)
- Краткое описание (textarea, макс 200 символов)
- Категория (select: Бизнес, Психология, Фитнес, IT, Языки, Творчество, Другое)

Шаг 2 — "Бренд":
- Загрузка логотипа (drag-and-drop)
- Выбор цветовой схемы (6 предустановленных тем + custom)
- Превью как будет выглядеть сайт школы

Шаг 3 — "Домен":
- Поддомен: [input].levelup-academy.ru (с проверкой доступности в реальном времени)
- Или свой домен (input, с инструкцией по настройке DNS)
...
Дизайн: чистый, пошаговый, с анимациями переходов между шагами.
```

**4. Генерация экранов для школы (*.levelup-academy.ru):**
...
1. KPI-карточки: активных студентов, курсов, доход за месяц (₽), средний рейтинг
2. График "Новые студенты за 30 дней" (area chart)
3. "Последние записи на курсы" — таблица (студент, курс, дата, статус оплаты)
4. "Популярные курсы" — bar chart (название курса → количество студентов)
```
...
Layout:
- Левая панель (сворачиваемая): список модулей и уроков курса. Каждый урок показывает: иконку типа (video/text/quiz), название, длительность, статус (✅ пройден / 🔵 текущий / ⚪ заблокирован)
- Основная область: содержимое текущего урока
  - Видеоурок: плеер (16:9), кнопки скорости (0.75x, 1x, 1.25x, 1.5x, 2x)
  - Текстовый урок: форматированный текст с изображениями
  - Под уроком: кнопки "← Предыдущий" и "Следующий →"
  - Блок комментариев/вопросов

Хедер: название курса, прогресс (progress bar, XX% пройдено), кнопка "Вернуться к курсам"
```

**5. Генерация экранов для Ассоциации (levelup-association.ru):**
...
3. Статистика: 4 числа в ряд (членов ассоциации, сертифицированных коучей, проведённых мероприятий, часов супервизии)
4. Уровни членства: 3 карточки (Ассоциированный член / Сертифицированный коуч / Мастер-коуч) с описанием требований и привилегий
5. Ближайшие мероприятия: карточки (название, дата, формат онлайн/офлайн, спикер, кнопка "Зарегистрироваться")
6. Публичный реестр коучей: поле поиска + превью-таблица (фото, имя, уровень, город, специализация)
7. Футер

Стиль: строгий, профессиональный. Цвета: тёмно-синий (#1e3a5f), золотой (#c9a84c) для акцентов.
```

**6. Экспортировать и адаптировать код:**
...
b) **Разложить компоненты по монорепозиторию:**

```bash
# Скопировать страницы платформы:
cp -r lovable-export/src/pages/* apps/platform/src/pages/
cp -r lovable-export/src/components/* apps/platform/src/components/

# Общие UI-компоненты (кнопки, карточки, модалки) вынести в packages/ui:
mkdir -p packages/ui/src/components
# Перенесите общие компоненты: Button, Card, Modal, Input, Avatar, Badge и т.д.
```

c) **Очистить сгенерированный код:**
   - Удалить захардкоженные данные, заменить на пропсы/стейт
   - Привести именование файлов в единый стиль (PascalCase для компонентов)
   - Удалить неиспользуемые импорты
   - Заменить `<a href>` на `<Link>` из React Router
   - Добавить TypeScript-типы для пропсов

d) **Проверить что всё работает:**

```bash
cd ~/levelup-platform/apps/platform
npm run dev
# Открыть http://localhost:5173 — убедиться что страницы отображаются
```

#### Зачем это нужно
lovable.dev экономит 30–40% времени на вёрстке. Вместо ручной вёрстки каждой кнопки и формы, вы получаете готовый React-код с Tailwind-стилями. Код совместим с вашим стеком (React + Tailwind + shadcn/ui), поэтому адаптация минимальна.

#### Приоритет экранов (что генерировать первым)

Для MVP генерируйте в таком порядке:

1. **Платформа (обязательно для MVP):**
   - Лендинг → Авторизация → Дашборд коуча → Дашборд клиента → Профиль коуча

2. **Академия (обязательно для MVP):**
   - Лендинг + каталог школ → Wizard создания школы

3. **Школа (обязательно для MVP):**
   - Админка школы → Страница курса для студента

4. **Ассоциация (можно отложить на фазу 2):**
   - Лендинг → Реестр коучей

> **Совет:** Не тратьте на генерацию больше 2–3 дней. Лучше сгенерировать базовые экраны и доработать вручную, чем пытаться получить идеальный результат из lovable.dev.
