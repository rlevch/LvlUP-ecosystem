# Архитектура платформы LevelUP

## 1. Резюме требований

Экосистема LevelUP состоит из **трёх отдельных сайтов на разных доменах**, объединённых общим бэкендом:

- **levelup-platform.ru** — Платформа-маркетплейс (продажа услуг коучей, сессии, магазин)
- **levelup-academy.ru** — SaaS-сервис для создания онлайн-школ (по модели GetCourse), каталог школ
- **levelup-association.ru** — Сайт конкретной ассоциации LevelUP (членство, сертификация, стандарты)
...
**Ключевое изменение концепции:** Академия (`levelup-academy.ru`) — это не одна конкретная школа, а **платформа-конструктор онлайн-школ**. Каждый владелец школы (school_owner) получает:
- Собственный поддомен (`my-school.levelup-academy.ru`) или кастомный домен (`my-school.ru`)
...
| Три отдельных сайта | Три React SPA на разных доменах (levelup-platform.ru, levelup-academy.ru, levelup-association.ru) + общий бэкенд в монорепозитории |
| **Мультитенантность (Академия-как-сервис)** | Каждая школа — отдельный tenant. Единая БД с `tenant_id` во всех academy-таблицах + RLS-изоляция. Wildcard-поддомены `*.levelup-academy.ru` + кастомные домены через Traefik + Let's Encrypt. Движок тем для кастомизации бренда каждой школы |
| Кастомные домены школ | Traefik с автоматическим TLS (Let's Encrypt, DNS-01 challenge через Selectel DNS API). Поддержка поддоменов `*.levelup-academy.ru` и собственных доменов владельцев школ |
...
│  │ levelup-platform.ru       │  │ levelup-association.ru│  │ academy-        │  │
...
│  │  my-school.levelup-academy.ru / my-school.ru                         │    │
│  │  ┌──────────────┐  ┌──────────────┐  ┌───────────────────────┐       │    │
...
│   │     levelup-platform.ru → Platform SPA                             │   │
│   │     levelup-association.ru → Association SPA                    │   │
│   │     levelup-academy.ru → Academy SPA (каталог школ)            │   │
│   │     *.levelup-academy.ru → School SPA (tenant по поддомену)    │   │
│   │     custom domain → tenant.school_domains → School SPA          │   │
│   │  2. Для школ: добавляет X-Tenant-ID в запрос                     │   │
│   │  3. Wildcard TLS: *.levelup-academy.ru (Let's Encrypt)          │   │
│   │     + TLS для levelup-platform.ru, levelup-association.ru         │   │
...
│Storage │ │Supavisor│ │ tenant   │ │ tenant- │ │          │
...
Внешние сервисы (РФ):
  ...
  │   ├── levelup-platform.ru → VPS #1
  │   ├── levelup-association.ru → VPS #1
  │   ├── levelup-academy.ru → VPS #1
  │   └── *.levelup-academy.ru → VPS #1 (wildcard для школ)
  └── Let's Encrypt (wildcard *.levelup-academy.ru + on-demand TLS для кастомных доменов)
```
...
```
Браузер: my-school.levelup-academy.ru/courses
    │
    ▼
Traefik (reverse proxy)
    │ Host: my-school.levelup-academy.ru
    ▼
...
|---|---|---|
| **Поддомен академии** | `my-school.levelup-academy.ru` | Wildcard DNS `*.levelup-academy.ru` → VPS. Traefik извлекает slug из поддомена |
| **Кастомный домен** | `academy.my-school.ru` | Владелец добавляет CNAME → `proxy.levelup-academy.ru`. Traefik получает TLS через Let's Encrypt (HTTP-01 challenge) |
...
└── /subscription       # Тариф школы на платформе LevelUP
...
5. Email:
   - From: "{school_name} <noreply@levelup-academy.ru>" (или кастомный sender)
...
-- slug: уникальный идентификатор для поддомена (my-school.levelup-academy.ru)
...
-- Для subdomain: автоматически active (*.levelup-academy.ru wildcard)
-- Для custom: требуется DNS-верификация (CNAME → proxy.levelup-academy.ru)
...
```
1. Пользователь заходит на my-school.levelup-academy.ru
...
levelup-platform/
├── apps/
│   ├── platform/               # 🔄 SPA маркетплейса (levelup-platform.ru)
│   │   ├── src/
...
│   │   └── package.json
│   │
│   ├── academy/                # 🆕 SPA академии (levelup-academy.ru)
│   │   ├── src/
│   │   │   ├── app/
│   │   │   ├── modules/
│   │   │   │   ├── auth/          # Авторизация
...
│   │   └── package.json
│   │
│   ├── association/            # 🆕 SPA ассоциации (levelup-association.ru)
│   │   ├── src/
│   │   │   ├── app/
│   │   │   ├── modules/
│   │   │   │   ├── auth/          # Авторизация
...
│   │   └── package.json
│   │
│   ├── school/                  # SPA школы (*.levelup-academy.ru / кастомный домен)
│   │   ├── src/
│   │   │   ├── app/
...
│   │   ├── coaching.ts          # levelup-platform.ru config
...
**Контекст 1: Платформа-маркетплейс (levelup-platform.ru)**
...
**Контекст 2: Академия (levelup-academy.ru)**
...
**Контекст 3: Ассоциация (levelup-association.ru)**

```
levelup-association.ru/
├── /                           # Главная страница ассоциации LevelUP
├── /auth/*                     # Вход, регистрация
├── /membership                 # Членство и уровни
...
**Контекст 4: Сайт школы (my-school.levelup-academy.ru / my-school.ru)**
...
Браузер → Host: ?
           │
           ├── levelup-platform.ru → Загрузить Platform SPA
           ├── levelup-academy.ru (без поддомена) → Загрузить Academy SPA
           ├── levelup-association.ru → Загрузить Association SPA
           ├── *.levelup-academy.ru → Извлечь slug → Загрузить School SPA (tenant)
...
| **Коуч** | levelup-platform.ru | Мои услуги, расписание, клиенты, сессии, магазин материалов, блог |
| **Клиент** | levelup-platform.ru | Каталог коучей, мои сессии, абонементы, диагностика |
| **Член ассоциации** | levelup-association.ru | Членство, сертификаты, реестр, события |
| **Анонимный пользователь** | levelup-platform.ru | Анонимный чат с волонтёром-«слушателем» (по модели 7 Cups) |
...
- Быть **коучем** на levelup-platform.ru и **владельцем школы** на levelup-academy.ru
- Быть **членом ассоциации** на levelup-association.ru и **студентом** в школе на levelup-academy.ru
...
**LiveKit — единственная видеоплатформа** в экосистеме LevelUP. Self-hosted на выделенном VPS Selectel в Москве.
...
- Клиент переходит по персональной ссылке коуча (например, `levelup-platform.ru/coach/ivanov` — на платформе)
...
> **Примечание:** эта секция описывает инфраструктуру для MVP (коучинг-экосистема: levelup-platform.ru + levelup-academy.ru + levelup-association.ru). Расчёт для будущих платформ (1000+ одновременных per platform) — см. Секцию 14.5.
...
│   ├── TLS: levelup-platform.ru, levelup-academy.ru, levelup-association.ru
│   ├── Wildcard TLS: *.levelup-academy.ru (Let's Encrypt, DNS-01 через Selectel API)
│   ├── On-demand TLS: для кастомных доменов школ (HTTP-01 challenge)
│   ├── Domain Router: levelup-platform.ru → Platform, levelup-association.ru → Association
│   ├── Tenant Router: *.levelup-academy.ru → School SPA (tenant по Host-заголовку)
...
- 🆕 Wildcard A-запись: *.levelup-academy.ru → IP VPS #1
  (все поддомены школ автоматически попадают на VPS)
- A-записи: levelup-platform.ru, levelup-association.ru, levelup-academy.ru → IP VPS #1
- 🆕 Для кастомных доменов школ: владелец создаёт CNAME → proxy.levelup-academy.ru
...
3. **API отвечает быстро:**
   ...
   - Supavisor (встроенный в Supabase v2 пул соединений) — нет задержки на установление соединения
...
max_connections = 200          # через Supavisor = до 500 клиентов
...
**Цель:** запуск levelup-academy.ru — пользователи могут создавать свои школы.

- 🆕 Три домена: levelup-platform.ru, levelup-academy.ru, levelup-association.ru (пока academy — основной)
...
- 🆕 Поддомены школ: *.levelup-academy.ru (wildcard DNS + Traefik)
...
- Мессенджер (базовый, без E2E, tenant-scoped)
- Видеоконференции через LiveKit (1-к-1 и групповые) — для школ и платформы
- 🆕 Видеосессии школы: преподаватель проводит лекции/вебинары/супервизии со студентами (LiveKit)
- 🆕 Библиотека школы: загрузка и каталогизация учебных материалов (tenant-scoped)
- Оплата курсов через ЮKassa + Split Payments (расщепление платежей школа↔платформа)
- Email-уведомления (с брендом школы)
- 🆕 Тарифные планы для школ (4 уровня: Стартовый / Базовый / Про / Бизнес)
- 🆕 Кастомные домены школ (CNAME + on-demand TLS)
- 🆕 Полный Theme Engine (8–10 пресетов, 6 цветов, шрифты, скругления, color picker)
- 🆕 Кастомный CSS для школ (тариф Про+)
- 🆕 Тесты/квизы (конструктор, 5 типов вопросов, автопроверка)
- 🆕 Рабочие листы (worksheets) — интерактивные формы
...
- 🆕 Аналитика школы: доход, студенты, конверсии, когорты, графики
- 🆕 Чат школы (tenant-scoped каналы: курсовые группы, личные, объявления)
- 🆕 Блог школы
- 🆕 Подписки на курсы (рекуррентные платежи)
- 🆕 Статические страницы школы (конструктор блоков)
- 🆕 Шаблоны сертификатов для выпускников школ
- 🆕 Каталог школ на levelup-academy.ru

### Фаза 2 — Маркетплейс коучей (2–3 месяца)
**Цель:** запуск levelup-platform.ru (маркетплейс коучей).

- Каталог коучей с поиском и фильтрами
- Календарь и бронирование сессий
- Личные кабинеты (коуч, клиент)
- Запись сессий (с согласием, LiveKit Egress)
- Абонементная система
- Рейтинги и отзывы
- Виртуальная приёмная
- Игровой движок (МАК-карты, кубики, поля)
- Коучинговые программы
- Автоматические выплаты коучам
- Промокоды
- Магазин материалов

### Фаза 3 — Ассоциация (2–2.5 месяца)
**Цель:** запуск levelup-association.ru.
...
- AI-помощник (отчёты, журнал, помощь)
- Интерактивная доска (tldraw)
- Карточки (flashcards)
- 2FA
- CRM школы: лиды, воронки продаж
- Email-рассылки школы (кампании, сегменты)
- 🆕 API для школ (тариф Бизнес): вебхуки, REST API для интеграций
- 🆕 White-label email (отправка с домена школы)
- 🆕 Автовебинары для школ
...
| 🆕 Wildcard SSL | Let's Encrypt: *.levelup-academy.ru + 3 основных домена (DNS-01 через Selectel API) | бесплатно |
...
| Нагрузка на PostgreSQL при росте | Средняя | Read-replica, материализованные представления, Supavisor |
...
Помимо levelup-platform.ru (маркетплейс коучей), планируется запуск двух дополнительных платформ-маркетплейсов:

- **Платформа для психологов** — отдельный домен, аналогичный функционал levelup-platform.ru (каталог специалистов, бронирование, видеосессии, магазин, AI-помощник)
...
└─────────────────┬───────────────────┬───────────────────┬───────────────┘
     ...
     │ levelup-platform.ru   │ │ [domain].ru      │ │ [domain].ru      │
     ├─────────────────────┤ ├──────────────────┤ ├──────────────────┤
...
export const config: PlatformConfig = {
  id: 'coaching',
  domain: 'levelup-platform.ru',
  name: 'LevelUP',
  supabaseUrl: 'https://api.levelup-platform.ru',
  supabaseAnonKey: '...',
  livekitUrl: 'wss://livekit.levelup-platform.ru',
...
Сборка: `PLATFORM=coaching turbo build --filter=platform` → бандл для levelup-platform.ru.
...
5. **Не завязываться на домен** — все URL, email-шаблоны, ссылки должны использовать `config.domain`, а не строковый литерал `levelup-platform.ru`
